...
# Files
TARGET = cicero
SRCFILES = $(SRCDIR)/main.c $(SRCDIR)/expr.c $(SRCDIR)/interp.c $(SRCDIR)/value.c $(SRCDIR)/parser/sexp.c $(SRCDIR)/values/int.c $(SRCDIR)/values/func.c
OBJFILES = $(OBJDIR)/main.o $(OBJDIR)/sexp.o $(OBJDIR)/expr.o $(OBJDIR)/interp.o $(OBJDIR)/value.o $(OBJDIR)/int.o $(OBJDIR)/func.o
...
$(OBJDIR)/int.o: $(SRCDIR)/values/int.c $(INCDIR)/values/int.h $(INCDIR)/value.h $(INCDIR)/values/type.h $(INCDIR)/values/int.h
	@mkdir -p $(OBJDIR)  # Create obj directory if it doesn't exist
	$(CC) $(CFLAGS) $(INCLUDES) -c $(SRCDIR)/values/int.c -o $(OBJDIR)/int.o

$(OBJDIR)/func.o: $(SRCDIR)/values/func.c $(INCDIR)/values/func.h $(INCDIR)/value.h $(INCDIR)/values/type.h $(INCDIR)/values/func.h
	@mkdir -p $(OBJDIR)  # Create obj directory if it doesn't exist
